...
(function () {
  ...
  // ----- Meteoro do Pedro (cheat: cd 0, fogo + gelo) ------------------------

  // Só permite pra quem entrou com nick "pedro"
  function isPedro() {
    const n = (S.player.nickname || '').trim().toLowerCase();
    return n === 'pedro';
  }

  // Acha o mob mais próximo dentro de N tiles. Se não tiver, usa um ponto à frente.
  function pickMeteorTarget() {
    ...
    S.mobs.forEach(function (m) {
      ...
      if (d < bestD && d <= maxPx) { bestD = d; best = m; }
    });
    if (best) return { x: best.x, y: best.y };
    // sem mob por perto: cai no mouse, ou na frente
    if (S.input && S.input.mouseWorldX != null) {
      return { x: S.input.mouseWorldX, y: S.input.mouseWorldY };
    }
    let dx = 0, dy = 0;
    if (p.facing === 'right') dx = 4 * TILE;
    else if (p.facing === 'left') dx = -4 * TILE;
    else if (p.facing === 'up') dy = -4 * TILE;
    else dy = 4 * TILE;
    return { x: p.x + dx, y: p.y + dy };
  }

  function pedroMeteor() {
    if (!isPedro()) {
      if (window.GTA.UI) window.GTA.UI.toast('Só o Pedro pode!', '#ff4f6f');
      return;
    }
    ...
    const tgt = pickMeteorTarget();
    const tx = tgt.x, ty = tgt.y;

    // Indicador no chão (mira do meteoro)
    if (window.GTA.Render) {
      window.GTA.Render.addAoE({
        x: tx, y: ty, r: 4 * TILE, life: 600,
        color: 'rgba(255,140,40,0.35)', kind: 'meteorMark',
      });
    }

    // Anima o meteoro caindo: partículas grandonas vindo de cima
    const fallMs = 500;
    const startY = ty - 600;
    const steps = 22;
    for (let i = 0; i < steps; i++) {
      const t = i / steps;
      window.GTA.Render && window.GTA.Render.particle({
        x: tx + (Math.random() - 0.5) * 16,
        y: startY + (ty - startY) * t,
        vx: (Math.random() - 0.5) * 30,
        vy: 200 + Math.random() * 100,
        life: fallMs * (1 - t) + 100,
        color: i % 2 ? '#ff8a3a' : '#9be7ff',
        size: 4, kind: 'meteor', gravity: 0,
      });
    }

    // Impacto agendado: explosão + dano + freeze depois do fallMs
    setTimeout(function () { meteorImpact(tx, ty); }, fallMs);
  }

  function meteorImpact(tx, ty) {
    const rPx = 4 * TILE;

    // Dano massivo + freeze em todos os mobs no raio
    for (const m of S.mobs.values()) {
      if (m.hp <= 0) continue;
      if (U.dist(m.x, m.y, tx, ty) <= rPx) {
        // congela primeiro (fica amarelinho de stun por 2s)
        m.stunUntil = S.now + 2000;
        m.slowUntil = S.now + 2000;
        // dano insano: mata até golem
        hitMob(m, 99999, '#9be7ff');
        // mais partículas de gelo no corpo do mob
        if (window.GTA.Render) {
          for (let i = 0; i < 14; i++) {
            window.GTA.Render.particle({
              x: m.x, y: m.y,
              ...
              color: i % 2 ? '#9be7ff' : '#cfefff',
              size: 3, kind: 'ice', gravity: 240,
            });
          }
        }
      }
    ...
    // FX da explosão
    if (window.GTA.Render) {
      // anel de fogo + anel de gelo
      window.GTA.Render.addAoE({
        x: tx, y: ty, r: rPx, life: 700,
        color: 'rgba(255,120,40,0.5)', kind: 'arcane',
      });
      window.GTA.Render.addAoE({
        x: tx, y: ty, r: rPx * 0.7, life: 700,
        color: 'rgba(155,231,255,0.5)', kind: 'arcane',
      });
      // explosão de partículas: metade fogo, metade gelo
      for (let i = 0; i < 60; i++) {
        const ang = Math.random() * Math.PI * 2;
        ...
        window.GTA.Render.particle({
          ...
          color: fogo ? (Math.random() < 0.5 ? '#ff5a1f' : '#ffc24a') : (Math.random() < 0.5 ? '#9be7ff' : '#dff5ff'),
          size: 3, kind: 'spark', gravity: 0,
        });
      }
      // shake forte
      window.GTA.Render.shake(14, 400);
    }
    if (window.GTA.UI) window.GTA.UI.toast('METEORO!', '#ff8a3a');
  }
